...
static TAutoConsoleVariable<int32> CVarRuntimeShadingPath(
	TEXT("r.Runtime.ShadingPath"),
	0,
	TEXT("Use Runtime ShadingPath."),
	ECVF_Default);
...
public:
		...
		bool bAllowSplitScreenDebug) const
	{
		...
		for (FSceneViewFamily* ViewFamily : ViewFamilies)
		{
			...
			if (CVarRuntimeShadingPath.GetValueOnGameThread())
			{
				OutRenderers.Add(new RuntimeRender(ViewFamily, HitProxyConsumer));
			}
			else
			{
